...
static double BLAST_Powi(double x, int32_t n)
{
   double   y;
...
static int16_t BLAST_Gcd(int16_t a, int16_t b)
{
	int16_t   c;
...
/*
Define the Python interface stuff
*/
static PyObject* karlin_estimateParameters(PyObject* self, PyObject *args)
{
    int obs_min = 0;
    int obs_max = 0;
    PyObject *score_obj;
    
    if (!PyArg_ParseTuple(args, "iiO", &obs_min, &obs_max, &score_obj))
        return NULL;
        
    PyObject *score_freq = PyArray_FROM_OTF(score_obj, NPY_DOUBLE, NPY_IN_ARRAY);

    if (score_freq == NULL)
    {
        Py_XDECREF(score_freq);
        return NULL;
    }

    double *score_array = (double*)PyArray_DATA(score_freq);
    
    double results[3] = {-1.0, -1.0, -1.0};
    double *lambda = results;
    double *entropy = lambda + 1;
    double *k = entropy + 1;
    ...
    /*
    Implicit cast to correct int size for observed
    min/max score values
    */
    int16_t s_obs_min = obs_min;
    int16_t s_obs_max = obs_max;

    ComputeKarlinAltschulParameters(score_array,
                                    s_obs_min, s_obs_max,
                                    lo_bound, hi_bound,
                                    lambda, entropy, k);

    Py_DECREF(score_freq);
        
    PyObject* ret = Py_BuildValue("ddd", *lambda, *entropy, *k);

    return ret;
}

static char karlin_docstring[] =
    "This module provides an interface for calculating Karlin-Altschul parameters. "
    "The parameter estimation routines were extracted from NCBI BLAST v2.7.1. "
    "The interface accepts Python int and float types, and turns them into "
    "int16 (!) and double.";

static char estimateParameters_docstring[] =
   "estimateParameters( ): Estimate Karlin-Altschul parameters lambda and K and compute entropy H";

/*
Module's method definition table
*/
static PyMethodDef KarlinMethods[] = {
   {"estimateParameters", karlin_estimateParameters, 
      METH_VARARGS, estimateParameters_docstring},
      {NULL, NULL, 0, NULL}
};
...
*/
static struct PyModuleDef karlinmodule = {
    PyModuleDef_HEAD_INIT,
    "karlin",   /* name of module */
    karlin_docstring, /* module documentation, may be NULL */
...
{
    import_array();
    return PyModule_Create(&karlinmodule);
}